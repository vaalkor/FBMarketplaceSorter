...
{
    aText = a.childNodes[0].childNodes[1].childNodes[1].innerText.split('·')[1].trim();
    bText = b.childNodes[0].childNodes[1].childNodes[1].innerText.split('·')[1].trim();
...
var observer = new MutationObserver(function(mutations){
    if(mutations.filter(m=>m.type === "childList").length > 0)
    {
        ...
        {
            var grps = [[marketboxes[0]]];
            
            for(let i=1;i<marketboxes.length;i++)
            {
                if(marketboxes[i].parentElement == grps[grps.length-1][0].parentElement)
                {   
                    grps[grps.length-1].push(marketboxes[i])
                }
                else
                {
